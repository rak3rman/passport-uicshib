  ...
  <div class="min-h-full flex">
    <div class="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:flex-none lg:px-20 xl:px-24">
      <div class="mx-auto w-full max-w-sm lg:w-96">
        <div>
          <img class="h-12 w-auto" src="/images/uic_logo.png" alt="Workflow" />
    ...
    <div class="hidden lg:block relative w-0 flex-1">
      <img class="absolute inset-0 h-full w-full object-cover" src="/images/uic_arc1.jpeg" alt="" />
    </div>
  </div>
</template>
<script>
  ...
  methods: {
    login: async function(e) {
      ...
      const { user } = await useFetch('/api/login', {
        method: 'POST',
        body: {
          email: e.target.elements.netid.value,
